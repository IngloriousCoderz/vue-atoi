...

{
  ...
  // iterating on objects - new style
...

{
  ...
  numbers.forEach((item, index, arr) => log(item, index, arr));
}

{
  const log = (item, index, arr) => console.log(item, index, arr);

  const doLog = (item, index, arr) => log(item, index, arr);
...

{
  ...
  numbers.forEach((item) => {
    summation += item; // update
  });
  console.log(summation); // return
}

{
  let summation = numbers[0];
  numbers.slice(1).forEach((item) => {
    summation += item;
  });
...

{
  ...
  numbers.forEach((item) => {
    const squared = square(item);
    squares.push(squared); // update
  });
  console.log(numbers, squares); // return
}
